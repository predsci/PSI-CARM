...
##
## Set the number of days from first reported deaths (30, 46, 60, 75, 90 and 105)
##

days_from_first_death = 105

##
## Number of unique values for R(t) - loop over all these values
##

if (days_from_first_death == 30 ) {
	nb_vec = 2:3
} else if (days_from_first_death == 45 || days_from_first_death == 60) {
	nb_vec = 2:4
} else {
	nb_vec = 2:5
}
...
dir_name_days_from_first_death = paste0('days_from_death1_',days_from_first_death,'/')
...
State_db_file = "../../data/State_data.rds"

out_list = readRDS(file=State_db_file)

# convert new data structure to existing values
# Note: data will now include Guam, Virgin Islands, and North Mariana Islands
# dates <- cases.list$dates
dates <- out_list$time_series[[1]]$date

# Remove United States states Mariana Islands from database structure as well as US total 

remove_index = which(out_list$locs$Country.Region == "United States" & out_list$locs$Province.State == "Northern Mariana Islands" | out_list$locs$Province.State == "")
...
## retrieve cases and deaths from out_list and trim by date_last
...
analysis_tbl = data.frame(country = country, prov_state = prov_state, Cum.Confirmed =rep(0.0, nc), Cum.Deaths = rep(0.0, nc), CFR = rep(0.0, nc), Deaths_rate=rep(0.0, nc), Confirmed_rate = rep(0.0, nc), Chi2 = rep(0.0, nc), Scaling1 = rep(0.0, nc), Scaling2 = rep(0.0, nc), Mean_noise = rep(0.0, nc), Mean_abs_noise = rep(0.0, nc), Mean_remainder = rep(0.0, nc), Mean_abs_remainder = rep(0.0, nc))
...
for (ii in nc_start:nc_end) {
	...
	for (inb in 1:length(nb_vec)) {
		...
		cat("\n Starting MCMCM Procedure for nb = ", nb, '\n\n')
	...
	print(" Date of 1St Reported Death")
	print(day_first_death)
	print(as.numeric(obs_all[save_day1:(day_first_death+days_from_first_death)]))
	
	obs <- as.numeric(obs_all[save_day1:(day_first_death+days_from_first_death)])

	obs_cases <- as.numeric(cases[ii, save_day1:(day_first_death+days_from_first_death)])

	obs_inc <- as.numeric(inc_all[save_day1:(day_first_death+days_from_first_death)])
	...
	#
	# Initial Guess for the values of R and the times in each value 
	#
		Rval = rep(0, nb)
		tn = rep(0, nb)
	...
	tn[nb] = 7
	cat('\n Initial Guess for Rval: \n')
	print(Rval)
	cat('\n Initial Guess for tn: \n')
	print(tn[1:nb-1])
	...
	file2 = paste0(outdir,"/usa_epi_data_",string_ii,".rds")
